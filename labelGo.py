...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def yolo_dialog_clicked(self, yoloName):
        ...
        if yoloName == "yolov7":
            # default dir {source}/crop/
            isCrop = False
            scrop_get = QMessageBox.question(self, 'Attention', '是否对目标进行裁减?', QMessageBox.Yes | QMessageBox.No)
    ...
    def set_yolo_model_dialog(self):
        # 实例化
        mb = QMessageBox(self)
        # 设置模态
        mb.setModal(True)
        # 设置窗口标题 及样式
        mb.setWindowTitle("Yolo选择")
        mb.setWindowFlag(Qt.WindowType.Window)
        # 设置提示图标
        mb.setIcon(QMessageBox.Icon.Information)
        # 设置标题
        mb.setText("Please Select Yolo Type!")  # 支持富文本
        mb.setTextFormat(Qt.TextFormat.RichText)
        mb.setInformativeText("yolov5 or yolov7")
        cancel_btn = mb.addButton("cancel", QMessageBox.ButtonRole.NoRole)
        yolov7_btn = mb.addButton("yolov7", QMessageBox.ButtonRole.NoRole)
        yolov5_btn = mb.addButton("yolov5", QMessageBox.ButtonRole.NoRole)
        mb.setDefaultButton(cancel_btn)
        mb.setEscapeButton(cancel_btn)
        mb.buttonClicked.connect(lambda btn: {
            self.yolo_dialog_clicked("yolov7")
        } if btn == yolov7_btn else {
            self.yolo_dialog_clicked("yolov5")
        } if btn == yolov5_btn else {
            mb.close()
        })
        # 设置主标题的交互

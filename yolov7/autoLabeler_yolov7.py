...
def start(weight_path, last_open_dir, isCrop):
    ...
    parser.add_argument('--source', type=str, default=last_open_dir, help='file/dir/URL/glob, 0 for webcam')
    ...
    parser.add_argument('--save-crop', nargs='?', const=True, default=isCrop, help='isCrop object --source/crop/')
...
@torch.no_grad()
def run(weights='yolov7_tiny.pt',  # model.pt path(s)
    ...
    # Directories
    # save_dir = Path(increment_path(Path(source) / name, exist_ok=exist_ok))  # increment run
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        if save_crop:
                            #print(source)
                            #print(Path(source) / 'crops' / f'{p.stem}.jpg')
                            # Path(source) / 'crops' / names[c] / f'{p.stem}.jpg'
                            save_one_box(xyxy, im0s, file=Path(source) / 'crops' / f'{p.stem}.jpg', BGR=True)
